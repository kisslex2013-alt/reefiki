...
| 4 | FORMAT | Нет обязательных полей frontmatter (см. `wiki/_schema.md`) или в `index.md` есть запрещённые/устаревшие поля (`importance`) | auto где значение выводится (`use_count: 0`, `last_used: null`, пересборка `index.md`); иначе manual |
...
4. Stage только затронутые файлы: `git add wiki/ .reefiki/`; затем `git commit -m "<project>: lint fixes (N issues)"`. Не используй `git add -A` — в repo могут быть чужие dirty changes.
